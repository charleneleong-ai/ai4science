...
ENV = Path(__file__).resolve().parents[2] / ".env"  # touchstone/.env
...
def verdict_bar(counts: Counter, title: str):
...
def set_verdict_summary(run, counts: Counter) -> None:
...
def log_ranked(run, ranked, reference, metal: str = "Ni2+") -> Counter:
    ...
    run.log({
        "designs": designs,
        "filtering": verdict_bar(counts, "Verifier filtering of the pool"),
    ...
    set_verdict_summary(run, counts)
    return counts
...
def candidate_metrics(result: dict) -> dict:
    ...
    m = {"reward": result.get("reward", 0.0), "consensus_weight": CONSENSUS_WEIGHT.get(result.get("consensus"), 0.0)}
...
def stack_rows(results: list[dict]) -> tuple[list[str], list[list], Counter]:
    ...
    for r in results:
        ...
        row += [stack_cell(by_stage[s]) if s in by_stage else "—" for s in stages]
        if has_stress:
            st = r.get("stress", {})
